...
# Alternative:
# STAND_MODEL_PATH = os.path.join(BASE_DIR, "models_td3_stand", "td3_stand_final.zip")
...
if __name__ == "__main__":
    ...
    if os.path.exists(STAND_MODEL_PATH):
        print(f"Loading TD3 standing checkpoint: {STAND_MODEL_PATH}")
        model = TD3.load(
            STAND_MODEL_PATH,
            env=train_env,
            device=DEVICE,
        ...
        print("Standing TD3 checkpoint not found. Starting walking TD3 from scratch.")
        model = TD3(
            policy="MlpPolicy",
            env=train_env,
            learning_rate=3e-4,
