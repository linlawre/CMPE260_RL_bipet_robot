...
class BipedalBulletEnv(gym.Env):
    ...
    def _compute_stand_reward(self, action):
        ...
        height = base_pos[2]
        vx, vy, vz = base_lin_vel
        wx, wy, wz = base_ang_vel

        # Keep a meaningful positive survival signal
        alive_bonus = 1.0
        survival_bonus = 0.05

        # Stronger than before, but not overwhelming
        tilt_penalty = 1.5 * (abs(roll) + abs(pitch))
        height_penalty = 2.0 * max(0.0, self.target_base_height - height)

        ang_vel_penalty = 0.04 * (abs(wx) + abs(wy))
        drift_penalty = 0.08 * abs(vx) + 0.08 * abs(vy)

        yaw_penalty = 0.03 * abs(yaw)

        action_penalty = 0.0008 * np.sum(np.square(action))
        joint_vel_penalty = 0.0005 * np.sum(np.square(qd))

        contact_bonus = 0.06 * np.sum(contacts)
        both_feet_bonus = 0.05 if np.sum(contacts) >= 2.0 else 0.0
        ...
        info = {
            ...
            "both_feet_bonus": float(both_feet_bonus),
